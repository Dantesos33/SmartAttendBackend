...
class ClassroomAttendanceSystem:
    ...
    def _get_superres(self):
        ...
        with self._sr_lock:
            ...
            if not os.path.exists(SR_MODEL_PATH) or os.path.getsize(SR_MODEL_PATH) < 10000:
                ...
                for attempt in range(1, 4):
                    try:
                        ...
                        request = urllib.request.Request(SR_MODEL_URL, headers={"User-Agent": "SmartAttend/1.0"})
                        ...
                        if os.path.exists(tmp):
                            os.remove(tmp)
                        print(f"FSRCNN download attempt {attempt} failed: {exc}")
            ...
            try:
                ...
                print(f"FSRCNN initialization failed: {exc}")
                self._sr_model = None
    ...
    def _get_yunet_detector(self, input_size):
        ...
        with self._yunet_lock:
            ...
            if not os.path.exists(YUNET_MODEL_PATH) or os.path.getsize(YUNET_MODEL_PATH) < 10000:
                ...
                for attempt in range(1, 4):
                    try:
                        print(f"Downloading YuNet model (attempt {attempt}/3)...")
                        request = urllib.request.Request(YUNET_MODEL_URL, headers={"User-Agent": "SmartAttend/1.0"})
                        with urllib.request.urlopen(request, timeout=15) as resp, open(tmp, "wb") as out:
                            while chunk := resp.read(1024 * 1024):
                                out.write(chunk)
                        if os.path.getsize(tmp) < 10000:
                            raise RuntimeError("Downloaded file too small.")
                        ...
                        if os.path.exists(tmp):
                            os.remove(tmp)
                        print(f"YuNet download attempt {attempt} failed: {exc}")
            ...
            try:
                ...
                self._yunet_detector = cv2.FaceDetectorYN.create(
                    YUNET_MODEL_PATH, "", requested_size, 0.35, 0.30, 5000
                )
    ...
    def prepare_known_faces(self, db_encoding_rows=None, allowed_student_ids=None):
        ...
        if target_ids is None:
            target_ids = [
                int(stem) for stem in (
                    os.path.splitext(name)[0] for name in os.listdir(self.known_students_dir)
                    if name.lower().endswith((".png", ".jpg", ".jpeg")) and os.path.splitext(name)[0].isdigit()
                )
            ]
    ...
    def _detect_face_locations(self, rgb_image):
        ...
        try:
            ...
            print(f"HOG detection failed: {exc}")

        return self._dedupe_face_locations(raw, original.shape)
    ...
    def _assign_faces_to_students(
        ...
        for face_index, encoding in face_encodings_by_index.items():
            ...
            for idx, raw_distance in enumerate(distances):
                ...
                if distance <= 0.50:
                    ...
                    familiarity = 0.0
                options.append((distance, sid, self.known_face_names[idx], float(max(0.0, min(0.99, familiarity)))))
        ...
        for face_index, options in candidates_by_face.items():
            if not options:
                continue
            best_distance, sid, name, confidence = options[0]
            if confidence >= float(min_confidence):
                candidates.append((best_distance, face_index, sid, name, confidence))
    ...
    def _encode_face_one_at_a_time(self, rgb_image, location):
        try:
            ...
            fh = max(1, bottom - top)
            fw = max(1, right - left)
            pad_y, pad_x = int(fh * 0.35), int(fw * 0.35)
            t, b = max(0, top - pad_y), min(h, bottom + pad_y)
            l, r = max(0, left - pad_x), min(w, right + pad_x)
            ...
            encs = face_recognition.face_encodings(crop, known_face_locations=[(ct, cr, cb, cl)], num_jitters=0)
    ...
    def recognize_classroom(
        ...
        self.prepare_known_faces(db_encoding_rows=db_encoding_rows, allowed_student_ids=allowed_student_ids)

        has_registered_faces = bool(self.known_face_encodings)
        allowed_set = set(allowed_student_ids) if allowed_student_ids else None

        enrolled_with_faces = [sid for sid in allowed_set if sid in self.known_face_ids] if allowed_set else list(self.known_face_ids)

        classroom_image = face_recognition.load_image_file(classroom_image_path)
        classroom_image, _ = self._downscale_rgb(classroom_image, MAX_DETECT_EDGE)
        ...
        if has_registered_faces:
            assignments = self._assign_faces_to_students(face_encodings_by_index, tolerance=tolerance, allowed_set=allowed_set)

        present_student_ids = []
        unknown_faces = 0
        face_details = []
        annotated_bgr = cv2.cvtColor(classroom_image, cv2.COLOR_RGB2BGR)
        ...
        for face_index, location in enumerate(face_locations):
            ...
            if face_encoding is None or not has_registered_faces:
                ...
                student_id, name, confidence = assignments.get(face_index, (None, "Unknown", 0.0))
            ...
            cv2.rectangle(annotated_bgr, (left, label_y - h_txt - 4), (left + w_txt + 6, label_y + 4), color, cv2.FILLED)
            cv2.putText(annotated_bgr, label, (left + 3, label_y - 2), cv2.FONT_HERSHEY_DUPLEX, 0.55, (255, 255, 255), 1)

        absent_student_ids = [sid for sid in enrolled_with_faces if sid not in present_student_ids]

        attendance_data = {
            "date": datetime.now().strftime("%Y-%m-%d"),
            "time": datetime.now().strftime("%H:%M:%S"),
            "total_registered": len(enrolled_with_faces),
            ...
            "enrolled_with_face_photos": len(enrolled_with_faces),
            "recognition_available": has_registered_faces,
            "warning_message": None if has_registered_faces else "No enrolled students have profile photos registered on the server.",
