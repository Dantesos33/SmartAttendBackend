...
def check_enrollment_stage(
    ...
    # IMPORTANT: masked/hijab faces are never eligible for identity matching.
    # Detection, embeddings, recognition thresholds, and student assignment
    # logic remain unchanged for clear faces. We simply exclude masked faces
    # from the assignment input so they can NEVER make a student present.
    all_encodings = {
        int(face["face_index"]): np.array(face["encoding"], dtype=np.float64)
        for face in job["faces"]
        if face.get("encoding") and face.get("face_status") != "masked"
    }
    ...
    for face in job["faces"]:
        ...
        if status == "masked":
            # Hard safety rule: masked/hijab faces are always unrecognized.
            student_id = None
            name = "Unrecognized"
            confidence = 0.0
